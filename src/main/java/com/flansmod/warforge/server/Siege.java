...
public class Siege {
    ...
    private boolean isPlayerInWarzone(DimBlockPos siegeCampPos, EntityPlayerMP player) {
        System.out.println("PlayerPOS: " + player.getPosition());
        ...
        if ((playerChunkPos.x >= minChunkX && playerChunkPos.x <= maxChunkX) && (playerChunkPos.z >= minChunkZ && playerChunkPos.z <= maxChunkZ)) {
            System.out.println("Valid positon for " + player.getUniqueID() + ": true");
        } else {
            System.out.println("Valid positon for " + player.getUniqueID() + ": false");
        }
